...
export default function CasesView({ onOpenChat }: CasesViewProps) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-[var(--card)] rounded-3xl border border-[var(--border)] shadow-[var(--shadow-card)] overflow-hidden">
                                    ...
                                    <tr key={c.id} className="hover:bg-[var(--muted)]/20 transition-colors group cursor-pointer" onClick={() => setSelectedCase(c)}>
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                ...
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); onOpenChat?.(c.user.id); }}
                                                    className="p-2 hover:bg-[var(--card)] rounded-lg text-blue-600 shadow-sm"
                                                    title="Message Client"
                                                >
                                                    <MessageSquare size={16} />
                                                </button>
                                                <button className="p-2 hover:bg-[var(--card)] rounded-lg text-slate-600 shadow-sm" title="Quick View">
                                                    <Eye size={16} />
                                                </button>
                                                {c.status !== 'RESOLVED' && (
                                                    <button
                                                ...
                                                <div className="space-y-3">
                                                    {selectedCase.documents?.map((doc: any) => (
                                                        <div key={doc.id} className="flex items-center justify-between p-4 bg-[var(--muted)]/20 rounded-2xl border border-[var(--border)] hover:border-[var(--primary)] transition-colors cursor-pointer group shadow-sm">
                                                            ...
                                                            <a
                                                                href={doc.filePath.startsWith('http') ? doc.filePath : `${getApiUrl().replace('/api', '')}/${doc.filePath.replace(/\\/g, '/')}`}
                                                                target="_blank"
